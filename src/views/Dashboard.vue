...
const saveCategory = async () => {
  try {
    if (isEditingCategory.value) {
      const { error } = await supabase.from('categories').update(categoryForm.value).eq('id', categoryForm.value.id)
      if (error) throw error
    } else {
...
const deleteCategory = async (id) => {
  if (!confirm('Yakin ingin menghapus kategori ini? (Pastikan tidak ada produk yang memakainya)')) return
  try {
  ...
  <div class="h-screen bg-stone-50 flex overflow-hidden">
    ...
    <aside class="w-64 bg-[#2A1515] flex flex-col shadow-xl z-20 shrink-0">
      <div class="p-6 border-b border-[#4A2525]">
        <div class="font-serif text-2xl font-bold tracking-wider text-white">
          KOKOROKARA
        </div>
        <div class="text-[#D7BABA] text-xs font-bold uppercase tracking-widest mt-1">Sistem Admin</div>
      </div>
      
      <div class="flex-1 overflow-y-auto py-6 px-4 space-y-2">
        <button 
          @click="activeTab = 'products'" 
          :class="['w-full flex items-center px-4 py-3 rounded-xl font-bold transition-all text-sm', 
                    activeTab === 'products' ? 'bg-[#8B3A3A] text-white shadow-md' : 'text-[#D7BABA] hover:bg-[#4A2525] hover:text-white']"
        >
        ...
        <button 
          ...
          :class="['w-full flex items-center px-4 py-3 rounded-xl font-bold transition-all text-sm', 
                    activeTab === 'categories' ? 'bg-[#8B3A3A] text-white shadow-md' : 'text-[#D7BABA] hover:bg-[#4A2525] hover:text-white']"
        >
        ...
        <button 
          ...
          :class="['w-full flex items-center px-4 py-3 rounded-xl font-bold transition-all text-sm', 
                    activeTab === 'settings' ? 'bg-[#8B3A3A] text-white shadow-md' : 'text-[#D7BABA] hover:bg-[#4A2525] hover:text-white']"
        >
      ...
      <div class="p-4 border-t border-[#4A2525]">
        <button @click="handleLogout" class="w-full flex justify-center items-center px-4 py-3 rounded-xl border border-[#4A2525] font-bold text-[#D7BABA] hover:bg-[#4A2525] hover:text-white transition">
           Keluar / Logout
        </button>
      </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col overflow-hidden bg-[#FFFBF8]">
      ...
      <!-- Topbar Header -->
      <header class="bg-white border-b border-rose-100 flex items-center justify-between px-8 py-5 shrink-0 z-10">
        <div>
        ...
        <div v-if="activeTab === 'products'">
          <button @click="openAddProduct" class="bg-[#8B3A3A] text-white px-5 py-2.5 rounded-xl font-bold hover:bg-[#682a2a] transition shadow-md flex items-center">
        ...
        <div v-if="activeTab === 'categories'">
          <button @click="openAddCategory" class="bg-[#8B3A3A] text-white px-5 py-2.5 rounded-xl font-bold hover:bg-[#682a2a] transition shadow-md flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
            Tambah Kategori
          </button>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-8">
        <!-- Loading State -->
        <div v-if="isLoading" class="flex flex-col items-center justify-center h-full text-stone-500">
          <svg class="animate-spin h-8 w-8 text-[#8B3A3A] mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-rose-50 overflow-hidden">
              <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse whitespace-nowrap">
                  ...
                  <tbody>
                    <tr v-for="prod in products" :key="prod.id" class="border-b border-rose-50 hover:bg-rose-50/50 transition-colors">
                      <td class="p-5 flex items-center space-x-4">
                        <img v-if="prod.image_url" :src="prod.image_url" class="w-14 h-14 rounded-xl object-cover bg-stone-100 shadow-sm border border-rose-100" />
                        <div v-else class="w-14 h-14 rounded-xl bg-stone-100 flex items-center justify-center text-xs text-stone-400 border border-rose-100">Tanpa Foto</div>
                        <div>
                          <p class="font-bold text-[#4A2525] whitespace-normal line-clamp-1 max-w-[200px]">{{ prod.name }}</p>
                          <p class="text-xs text-stone-500 whitespace-normal line-clamp-1 max-w-[200px] mt-0.5">{{ prod.description }}</p>
                        </div>
                      </td>
                      <td class="p-5">
                      ...
                      <td class="p-5">
                        <div class="flex items-center space-x-2">
                          ...
                          <span v-else class="px-2.5 py-1 bg-red-50 border-red-100 text-red-600 rounded-md text-[11px] uppercase tracking-wider font-bold">Kosong</span>
                          
                          <span v-if="prod.is_preorder" class="px-2.5 py-1 bg-blue-50 text-blue-600 border border-blue-100 rounded-md text-[11px] uppercase tracking-wider font-bold">
                            PO <span class="text-blue-800">{{ prod.estimated_days }} hr</span>
                          </span>
                        </div>
                      </td>
                      <td class="p-5 text-right space-x-4">
                        <button @click="openEditProduct(prod)" class="text-stone-400 hover:text-[#8B3A3A] transition">
                        ...
                        <button @click="deleteProduct(prod.id)" class="text-stone-400 hover:text-red-500 transition">
                    ...
                    <tr v-if="products.length === 0">
                      <td colspan="5" class="p-10 text-center">
                         <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-rose-50 text-rose-300 mb-4">
                           <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" /></svg>
                         </div>
                         <p class="text-stone-500 font-medium">Belum ada produk apa pun di katalog.</p>
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-rose-50 overflow-hidden max-w-4xl">
              <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse whitespace-nowrap">
                  ...
                  <tbody>
                    <tr v-for="cat in categories" :key="cat.id" class="border-b border-rose-50 hover:bg-rose-50/50 transition-colors">
                      <td class="p-5 text-sm font-bold text-stone-400">#{{ cat.id }}</td>
                      <td class="p-5 text-sm font-bold text-[#4A2525]">{{ cat.name }}</td>
                      <td class="p-5"><span class="px-3 py-1 bg-stone-100 text-stone-600 rounded-lg text-xs">{{ cat.slug }}</span></td>
                      <td class="p-5 text-right space-x-4">
                        <button @click="openEditCategory(cat)" class="text-stone-400 hover:text-[#8B3A3A] transition">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        </button>
                        <button @click="deleteCategory(cat.id)" class="text-stone-400 hover:text-red-500 transition">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        </button>
                      </td>
                    </tr>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-rose-50 p-8">
              <form @submit.prevent="saveSettings" class="space-y-8">
                ...
                <div class="pb-6 border-b border-rose-50">
                   <h3 class="text-xl font-serif text-[#4A2525] mb-4">Pengaturan Checkout WhatsApp</h3>
                   <div class="bg-[#FFFBF8] p-5 rounded-xl border border-rose-100 flex items-start">
                     ...
                     <div>
                       <label class="block text-sm font-bold text-stone-700 mb-1">Nomor WhatsApp Tujuan</label>
                       <p class="text-xs text-stone-500 mb-3 leading-relaxed">Saat pelanggan klik "Checkout via WhatsApp" di keranjang, format pesan akan dikirim otomatis ke nomor ini. Selalu gunakan awalan angka <strong class="text-stone-700">62</strong> sebagai kode area Indonesia.</p>
                       <input v-model="settings.whatsapp_number" type="tel" placeholder="Contoh: 628123456789" class="w-full max-w-sm px-4 py-3 bg-white rounded-xl border border-rose-200 focus:outline-none focus:border-[#8B3A3A] focus:ring-1 focus:ring-[#8B3A3A]" />
                     </div>
                   </div>
                </div>
                ...
                <div class="pb-6">
                   ...
                   <div class="flex items-center space-x-4 bg-rose-50 p-5 rounded-xl border border-rose-100">
                     <div class="shrink-0 flex items-center justify-center p-2 bg-yellow-100 text-yellow-600 rounded-lg">
                       <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                     </div>
                     <div class="flex-1">
                       <label for="maintenance" class="block text-sm font-bold text-stone-700 mb-0.5 cursor-pointer">Mode Perawatan (Under Maintenance)</label>
                       <p class="text-xs text-stone-500">Aktifkan ini jika Anda sedang membuat perubahan besar pada situs atau menutup toko sementara sehingga toko online tidak bisa menerima pembeli baru.</p>
                     </div>
                     <div class="shrink-0">
                       <label class="relative inline-flex items-center cursor-pointer">
                          <input v-model="settings.is_maintenance" type="checkbox" id="maintenance" class="sr-only peer">
                ...
                <div class="flex justify-end pt-4 border-t border-rose-50">
                  <button type="submit" class="bg-[#8B3A3A] text-white px-8 py-3.5 rounded-xl font-bold hover:bg-[#682a2a] transition shadow-lg shadow-[#8B3A3A]/20">
                    Simpan Perubahan
                  </button>
                </div>
    ...
    <div v-if="showProductModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-[2rem] w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-rose-100">
        <div class="flex justify-between items-center p-6 border-b border-rose-50 bg-[#FFFBF8]">
          <h2 class="text-2xl font-serif text-[#4A2525]">{{ isEditingProduct ? 'Sunting Data Produk' : 'Buat Produk Baru' }}</h2>
          <button @click="showProductModal = false" class="text-stone-400 hover:text-red-500 transition-colors p-2 bg-white rounded-full border border-rose-100">
        ...
        <form @submit.prevent="saveProduct" class="flex-1 overflow-y-auto p-8 bg-white">
          <div class="grid grid-cols-2 gap-5">
            <div class="col-span-2">
              ...
              <input v-model="productForm.name" required placeholder="Contoh: Buket Mawar Cantik" class="w-full px-4 py-3 bg-[#FFFBF8] rounded-xl border border-rose-100 focus:outline-none focus:ring-1 focus:ring-[#8B3A3A]" />
            </div>
            
            <div class="col-span-2">
              <label class="block text-sm font-bold text-stone-700 mb-1">Deskripsi Singkat</label>
              <textarea v-model="productForm.description" rows="3" placeholder="Jelaskan detail singkat tentang produk ini..." class="w-full px-4 py-3 bg-[#FFFBF8] rounded-xl border border-rose-100 focus:outline-none focus:ring-1 focus:ring-[#8B3A3A]"></textarea>
            </div>
          ...
          <div class="bg-rose-50/50 p-6 rounded-2xl border border-rose-100 space-y-5 mt-6">
             <h4 class="text-sm font-bold text-[#8B3A3A] uppercase tracking-widest border-b border-rose-100 pb-2">Status Penjualan</h4>
            
            <label class="flex items-center space-x-3 cursor-pointer">
              <input v-model="productForm.is_available" type="checkbox" class="w-5 h-5 rounded border-rose-300 text-[#8B3A3A] focus:ring-[#8B3A3A]" />
              <div class="flex-1">
                 <span class="block text-sm font-bold text-stone-700">Tersedia untuk dijual</span>
                 <span class="block text-xs text-stone-500">Mencentang ini berarti pembeli bisa menambahkan produk ke keranjang.</span>
              </div>
            </label>
            
            <div class="pt-4 border-t border-rose-100">
              <label class="flex items-center space-x-3 cursor-pointer">
                <input v-model="productForm.is_preorder" type="checkbox" class="w-5 h-5 rounded border-rose-300 text-[#8B3A3A] focus:ring-[#8B3A3A]" />
                <div class="flex-1">
                   <span class="block text-sm font-bold text-stone-700">Aktifkan Pre-Order (PO)</span>
                   <span class="block text-xs text-stone-500">Gunakan sistem ini jika buket membutuhkan waktu pengerjaan berhari-hari.</span>
                </div>
              </label>
              
              <div v-if="productForm.is_preorder" class="mt-4 ml-8 flex items-center bg-white p-3 rounded-lg border border-rose-100 inline-flex">
                <label class="text-sm font-bold text-stone-700 mr-3">Estimasi Pengerjaan:</label>
                <div class="flex items-center">
                  <input v-model.number="productForm.estimated_days" type="number" min="1" class="w-16 px-2 py-1.5 border border-rose-200 rounded-lg text-center focus:ring-1 focus:ring-[#8B3A3A]" />
    ...
    <div v-if="showCategoryModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-[2rem] w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-rose-100">
        <div class="flex justify-between items-center p-6 border-b border-rose-50 bg-[#FFFBF8]">
          <h2 class="text-xl font-serif text-[#4A2525]">{{ isEditingCategory ? 'Sunting Kategori' : 'Tambah Kategori' }}</h2>
          <button @click="showCategoryModal = false" class="text-stone-400 hover:text-red-500 transition-colors p-2 bg-white rounded-full border border-rose-100">
        ...
        <form @submit.prevent="saveCategory" class="flex-1 overflow-y-auto p-6 bg-white space-y-4">
          <div>
            ...
            <input v-model="categoryForm.name" required placeholder="Contoh: Fresh Flowers" class="w-full px-4 py-3 bg-[#FFFBF8] rounded-xl border border-rose-100 focus:outline-none focus:ring-1 focus:ring-[#8B3A3A]" />
          </div>
          <div>
            <label class="block text-sm font-bold text-stone-700 mb-1">Slug (Link URL)</label>
            <input v-model="categoryForm.slug" required placeholder="Contoh: fresh-flowers" class="w-full px-4 py-3 bg-[#FFFBF8] rounded-xl border border-rose-100 focus:outline-none focus:ring-1 focus:ring-[#8B3A3A]" />
            <p class="text-xs text-stone-400 mt-1">Harus unik, gunakan huruf kecil dan tanpa spasi (boleh pakai tanda strip).</p>
          </div>
        </form>

        <div class="p-6 border-t border-rose-100 flex space-x-4 shrink-0 bg-[#FFFBF8]">
           <button type="button" @click="showCategoryModal = false" class="flex-1 py-3 bg-white border border-rose-100 text-stone-500 rounded-xl font-bold hover:bg-stone-50 transition">Batal</button>
           <button type="button" @click="saveCategory" class="flex-1 py-3 bg-[#8B3A3A] text-white rounded-xl font-bold hover:bg-[#682a2a] shadow-lg shadow-[#8B3A3A]/20 transition">Simpan</button>
        </div>
      </div>
    </div>
  </div>
